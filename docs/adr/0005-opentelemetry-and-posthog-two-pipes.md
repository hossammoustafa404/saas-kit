...
We instrument with the OpenTelemetry SDK (HTTP and Prisma, started before Nest) and export Traces, Logs, and auto HTTP Metrics to PostHog when a project token is set. Auth Events (`user signed up`, `email verified`, `user signed in`, `user signed out`) use `posthog-node` from AfterHooks. We do not treat spans, span events, or Logs as Events, and we do not send Events over OTLP.
