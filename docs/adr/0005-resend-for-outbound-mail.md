...
Resend rejects reserved test domains such as `example.com`. Outside production, the processor completes those jobs without calling Resend so local sign-up and e2e do not retry a send that cannot succeed. In production the skip is off — reserved and real recipients both go to Resend. Other 4xx responses (except rate limits) fail the job without retry.
