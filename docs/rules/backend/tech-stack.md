...
| Logging        | NestJS Logger; `AppLogger` (JSON stdout + OTLP) when tracing is on | `nestjs.md`             |
...
When tracing is on, Nest uses `AppLogger` (JSON stdout with `trace_id` / `span_id` when a span is active) and the same NodeSDK exports Logs to PostHog’s `/i/v1/logs` over OTLP (`@opentelemetry/exporter-logs-otlp-proto`) with the same Bearer project token. When the token is missing, keep Nest’s default Logger. **NEVER** `capture()` a Log. Log every HTTP request as it arrives (`Incoming Request: METHOD /path`) and its final status (`Outcoming Response: METHOD /path 200 OK`, or the same prefix for thrown 4xx/5xx) — skip Health. Warn includes the exception message; error includes the message and a `stack` field. Better-auth incoming and outgoing status is logged from `httpObservabilityPlugin` (`hooks.before` / `hooks.after`), not the Nest interceptor or filter. **NEVER** log query strings, bodies, cookies, passwords, tokens, or PII.
