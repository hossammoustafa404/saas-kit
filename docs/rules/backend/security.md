...
- Never trust client input. Validate HTTP payloads with Zod via `@stack/schemas`. See `validation.md`.
- Store secrets in server env only (`BETTER_AUTH_SECRET`, `DATABASE_URL`, OAuth keys). **NEVER** expose them to clients.
- Validate server env in `apps/server/src/shared/config/`. **NEVER** put secret or env schemas in `@stack/schemas` — `web` and `admin` could import them.
...
- Disable interactive API docs in production, or protect them behind auth. See `api-docs.md`.
