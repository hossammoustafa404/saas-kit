...
Document a consistent Nest HTTP exception envelope on endpoints that can fail (`statusCode`, `message`, `error`). Keep that schema with the exception filter in the owning module when one exists — **NEVER** in `shared/swagger/`. **NEVER** `shared/filters/`.
